...
async function main() {
  ...
  if (balance < 0.5 * anchor.web3.LAMPORTS_PER_SOL) {
    console.log(
      '⚠️  Warning: Balance is low. You may need to request an airdrop:',
    )
  ...
  console.log('\nExplorer Links:')
  console.log(
    `Mint:     https://explorer.solana.com/address/${mintPubkey.toBase58()}?cluster=devnet`,
  )
  console.log(
    `Metadata: https://explorer.solana.com/address/${metadataPda.toBase58()}?cluster=devnet`,
  )
  console.log(
    `Token:    https://explorer.solana.com/address/${tokenAccount.address.toBase58()}?cluster=devnet`,
  )
